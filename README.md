<html lang="th">
 ...
 <body>
  <div class="game-wrapper">
   <div class="game-container">
    ...
    <div id="playerInputSection" class="player-input-section">
     <h3>กรุณากรอกชื่อผู้เล่น</h3><input type="text" id="playerNameInput" class="player-input" placeholder="ชื่อของคุณ" aria-label="ชื่อผู้เล่น">
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; max-width: 600px; margin: 0 auto;">
       <div><label style="display: block; margin-bottom: 10px; font-weight: bold; color: #667eea;">สีสัตว์:</label>
        <div style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: center;">
          <button class="color-btn active" data-color="#FFB347" onclick="selectAnimalColor('#FFB347')" style="background: #FFB347;" aria-label="สีส้ม"></button>
          <button class="color-btn" data-color="#87CEEB" onclick="selectAnimalColor('#87CEEB')" style="background: #87CEEB;" aria-label="สีฟ้า"></button>
          <button class="color-btn" data-color="#FFB6C1" onclick="selectAnimalColor('#FFB6C1')" style="background: #FFB6C1;" aria-label="สีชมพู"></button>
          <button class="color-btn" data-color="#90EE90" onclick="selectAnimalColor('#90EE90')" style="background: #90EE90;" aria-label="สีเขียว"></button>
          <button class="color-btn" data-color="#DDA0DD" onclick="selectAnimalColor('#DDA0DD')" style="background: #DDA0DD;" aria-label="สีม่วง"></button>
          <button class="color-btn" data-color="#F0E68C" onclick="selectAnimalColor('#F0E68C')" style="background: #F0E68C;" aria-label="สีเหลือง"></button>
        </div>
       </div>
       <div><label style="display: block; margin-bottom: 10px; font-weight: bold; color: #667eea;">ประเภทสัตว์:</label>
        <div style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: center;">
          <button class="hair-btn active" onclick="selectAnimalType(event,'cat')" style="font-size: 32px;" aria-label="เลือกแมว">🐱</button>
          <button class="hair-btn" onclick="selectAnimalType(event,'dog')" style="font-size: 32px;" aria-label="เลือกสุนัข">🐶</button>
          <button class="hair-btn" onclick="selectAnimalType(event,'rabbit')" style="font-size: 32px;" aria-label="เลือกกระต่าย">🐰</button>
          <button class="hair-btn" onclick="selectAnimalType(event,'bear')" style="font-size: 32px;" aria-label="เลือกหมี">🐻</button>
        </div>
       </div>
      </div>
     ...
     <div id="gameArea" class="game-area hidden">
      ...
      <div id="mode1Content" class="hidden">
       ...
       <div class="drop-zones">
        <div class="drop-zone" id="acidDropZone" role="button" tabindex="0" aria-label="โซนกรด">
         <div class="drop-zone-label">
          กรด (Acid)
         </div>
         <div id="acidDropped"></div>
        </div>
        <div class="drop-zone" id="baseDropZone" role="button" tabindex="0" aria-label="โซนเบส">
         <div class="drop-zone-label">
          เบส (Base)
         </div>
    ...
    <div id="playerCharacter" class="player-character hidden" aria-hidden="true">
  ...
  <script>
    ...
    function selectAnimalType(evt, type) {
    ...
    const dataHandler = {
      onDataChanged(data) {
        // store top 10 (sorted by score desc)
        allScores = Array.isArray(data) ? data.sort((a, b) => b.score - a.score).slice(0, 10) : [];
        renderLeaderboard();
      }
    };
    ...
    function selectMode(mode) {
      ...
      // Freeze selection cards once chosen
    ...
    function loadQuestion() {
      ...
      // Make choice items interactive again
      document.querySelectorAll('.choice-item').forEach(el => {
        el.classList.remove('disabled', 'used');
        el.style.background = '';
    ...
    function loadMode1Question(question) {
      ...
      // clear drop zones
      document.getElementById('acidDropped').textContent = '';
      document.getElementById('baseDropped').textContent = '';
      ...
      allChoices.forEach(choice => {
        ...
        // classify roughly by string content
        if (choice.includes('H') && !choice.includes('OH') && !choice.includes('Na') && !choice.includes('K') && !choice.includes('Ca') && !choice.includes('Mg')) {
    ...
    function setupDropZones() {
      ...
      if (acidZone.dataset.listenersAdded === 'true') return;
      acidZone.dataset.listenersAdded = 'true';
      baseZone.dataset.listenersAdded = 'true';
      ...
      [acidZone, baseZone].forEach(zone => {
        ...
        // click handler to remove selected item (for accessibility)
        zone.addEventListener('click', () => {
          if (zone.id === 'acidDropZone' && selectedAcid && !selectedBase && !isAnswering) {
            removeAnswerFromZone('acid');
          }
          if (zone.id === 'baseDropZone' && selectedBase && !selectedAcid && !isAnswering) {
            removeAnswerFromZone('base');
          }
        });

        // key handler for accessibility: Enter or Space acts like click
    ...
    function handleChoiceClick(choice, element) {
      if (!selectedAcid) {
        ...
        document.getElementById('acidDropped').innerHTML = `<span class="dropped-item">${escapeHtml(choice)}</span>`;
        document.getElementById('acidDropZone').classList.add('filled');
      } else if (!selectedBase) {
        selectedBase = choice;
        element.classList.add('used');
        document.getElementById('baseDropped').innerHTML = `<span class="dropped-item">${escapeHtml(choice)}</span>`;
    ...
    async function endGame() {
      ...
      if (Array.isArray(allScores) && allScores.length >= 999) {
        ...
        if (window.dataSdk && window.dataSdk.create) {
          ...
          // give dataSdk a moment to push updated data via onDataChanged
          await new Promise(resolve => setTimeout(resolve, 1000));
        } else {
          // if no SDK, just simulate local store
          allScores = allScores || [];
          allScores.push({ id: Date.now().toString(), player_name: playerName, score: score, mode: modeText, completed_at: new Date().toISOString()});
          allScores = allScores.sort((a,b)=>b.score-a.score).slice(0, 10);
        }
      }
    ...
    function generateCompetitionSummary() {
      ...
      // find latest matching record for this player and score
      const playerRank = sameModeScores.findIndex(s => 
        s.player_name === playerName && s.score === score && s.mode === modeText
      ) + 1;
    ...
    function renderLeaderboard() {
      ...
      list.innerHTML = allScores.map((record, index) => `
        <li class="leaderboard-item">
          ...
          <span class="leaderboard-name">${escapeHtml(record.player_name)}</span>
          <span class="leaderboard-score">${record.score} คะแนน (${escapeHtml(record.mode)})</span>
        </li>
      `).join('');
    }
